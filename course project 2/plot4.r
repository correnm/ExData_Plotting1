...
## Find just the records related to coal
coal <- grep('*[cC]oal*',SCC$EI.Sector)
SCCcoal <- SCC[coal,]
## Now in the coal records also look for combustion
combustion <- grep('*[cC]omb*',SCCcoal$EI.Sector)
allData <- SCCcoal[combustion,]
coalCombustion <-allData[,1]
## Merge the two data sources based on the SCC code
coalEmissions <- subset(NEI,NEI$SCC %in% coalCombustion)

## Aggregate and sum the emissions by year
emissionsByYear<-aggregate(Emissions ~ year, data = coalEmissions, sum)
...
plot(xrange, yrange,  type="n", xlab="Year", ylab="Total Emissions (Coal Combustion)",
